...
- Python 3 on Windows.
- No runtime third-party Python package is required for normal GUI/CLI usage.
- PyInstaller is only needed when building an `.exe`.
...
The included `PMXTranslateEditor.spec` contains explicit Tkinter/Tcl/Tk paths for the current Python installation. If you build on another machine or another Python version, update those paths in the `.spec` file.
...
- Python 3 trên Windows.
- Không cần cài package Python bên thứ ba để chạy GUI/CLI thông thường.
- Chỉ cần PyInstaller nếu muốn đóng gói thành file `.exe`.
...
Cài PyInstaller trên máy build:
...
File `PMXTranslateEditor.spec` hiện có đường dẫn Tkinter/Tcl/Tk cụ thể cho môi trường Python hiện tại. Nếu build trên máy khác hoặc phiên bản Python khác, hãy cập nhật lại các đường dẫn trong file `.spec`.
...
- Windows 上的 Python 3。
- 正常使用 GUI/CLI 不需要第三方 Python 包。
- 只有在打包 `.exe` 时才需要 PyInstaller。
...
当前的 `PMXTranslateEditor.spec` 包含此 Python 环境的 Tkinter/Tcl/Tk 路径。如果在其他机器或其他 Python 版本上构建，请更新 `.spec` 文件中的路径。
...
- Windows 上の Python 3。
- 通常の GUI/CLI 実行にはサードパーティ製 Python パッケージは不要です。
- `.exe` を作成する場合のみ PyInstaller が必要です。
...
付属の `PMXTranslateEditor.spec` には、現在の Python 環境向けの Tkinter/Tcl/Tk パスが明示されています。別の PC や別の Python バージョンでビルドする場合は、`.spec` 内のパスを更新してください。
